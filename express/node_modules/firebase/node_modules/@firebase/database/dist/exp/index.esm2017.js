...
class ViewProcessor {
    ...
    applyServerOverwrite_(oldViewCache, changePath, changedSnap, writesCache, completeCache, filterServerNode, accumulator) {
        ...
        const newViewCache = oldViewCache.updateServerSnap(newServerCache, oldServerSnap.isFullyInitialized() || pathIsEmpty(changePath), serverFilter.filtersNodes());
    ...
    listenComplete_(viewCache, path, writesCache, accumulator) {
        ...
        const newViewCache = viewCache.updateServerSnap(oldServerNode.getNode(), oldServerNode.isFullyInitialized() || pathIsEmpty(path), oldServerNode.isFiltered());
        ...
        if (!removedWriteWasVisible) {
            ...
            if (writeToRemove.snap) {
                ...
                each(children, (childName) => {
                    this.visibleWrites_ = compoundWriteRemoveWrite(this.visibleWrites_, pathChild(writeToRemove.path, childName));
                });
            }
            return true;
        }
    ...
    addEventRegistration(query, eventRegistration) {
        ...
        if (serverCache != null) {
            ...
            subtree.foreachChild((childName, childSyncPoint) => {
                const completeCache = childSyncPoint.getCompleteServerCache(newEmptyPath());
                if (completeCache) {
...
class Tree {
    ...
    subTree(pathObj) {
        ...
        let child = this, next = pathGetFront(path);
        while (next !== null) {
...
class Repo {
        ...
        this.serverSyncTree_ = new SyncTree({
            startListening: (query, tag, currentHashFn, onComplete) => {
                this.server_.listen(query, currentHashFn, tag, (status, data) => {
                    ...
                    eventQueueRaiseEventsForChangedPath(this.eventQueue_, query.path, events);
                });
    ...
    setWithPriority(path, newVal, newPriority, onComplete) {
        ...
        this.server_.put(path.toString(), newNodeUnresolved.val(/*export=*/ true), (status, errorReason) => {
            ...
            eventQueueRaiseEventsForChangedPath(this.eventQueue_, path, clearEvents);
    ...
    update(path, childrenToMerge, onComplete) {
        ...
        each(childrenToMerge, (changedKey, changedValue) => {
            empty = false;
            changedChildren[changedKey] = resolveDeferredValueTree(pathChild(path, changedKey), nodeFromJSON$1(changedValue), this.serverSyncTree_, serverValues);
        });
        if (!empty) {
            const writeId = this.getNextWriteId_();
            ...
            this.server_.merge(path.toString(), childrenToMerge, (status, errorReason) => {
                ...
                eventQueueRaiseEventsForChangedPath(this.eventQueue_, affectedPath, clearEvents);
                this.callOnCompleteCallback(onComplete, status, errorReason);
            });
            each(childrenToMerge, (changedPath) => {
                const affectedPath = this.abortTransactions_(pathChild(path, changedPath));
                this.rerunTransactions_(affectedPath);
            });
            // We queued the events above, so just flush the queue here
        ...
        this.server_.onDisconnectMerge(path.toString(), childrenToMerge, (status, errorReason) => {
            if (status === 'ok') {
                each(childrenToMerge, (childName, childNode) => {
                    ...
                    this.onDisconnect_.remember(pathChild(path, childName), newChildNode);
                });
            }
            this.callOnCompleteCallback(onComplete, status, errorReason);
        });
